...
```{r}
# total richness - all species, all years (no synonyms or NAs)
length(unique(myspecies_coords$species[myspecies_coords$taxonomicStatus == "ACCEPTED" & is.na(myspecies_coords$species) == FALSE]))

# richness per year
richness_y <- myspecies_coords %>% 
  filter(taxonomicStatus == "ACCEPTED", is.na(species) == FALSE) %>% # remove synonyms and NAs
  select(species, year) %>% 
...
names(richness_order) <- c("Order", "Species Richness")
...
# rename for formatting
names(richness_diet) <- c("Diet", "Species Richness")
richness_diet$Diet <- str_to_title(richness_diet$Diet)
...
write.csv(richness_diet, file = here("data", "output", "richness_diet.csv"))

# order & number of species
write.csv(richness_order, file = here("data", "output", "richness_order.csv"))
